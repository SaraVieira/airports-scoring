...
class GoogleReviewsScraper:
    ...
    def navigate_to_place(self, driver: Chrome, url: str, wait: WebDriverWait) -> bool:
        ...
        # Step 1: Warm up - visit google.com with English locale to establish session cookies
        try:
            driver.get("https://www.google.com/?hl=en")
            time.sleep(2)
            self.dismiss_cookies(driver)
            # Also handle the full-page EU consent redirect (consent.google.com)
            if "consent.google" in driver.current_url:
                log.info("EU consent page detected, attempting to accept...")
                time.sleep(2)
                for selector in [
                    'button[jsname="hZCF7e"]',
                    'button[jsname="b3VHJd"]',
                    ...
                    try:
                        ...
                        for btn in btns:
                            if btn.is_displayed():
                                btn.click()
                                log.info(f"EU consent accepted via {selector}")
                                time.sleep(2)
                                break
                    except Exception:
                        continue
        ...
        if place_name:
            ...
            if lat and lng:
                search_url = f"https://www.google.com/maps/search/{search_query}/@{lat},{lng},17z?hl=en"
            else:
        ...
        driver.get(url)
        try:
            wait.until(lambda d: "google.com/maps" in d.current_url or "consent.google" in d.current_url)
        except TimeoutException:
            log.warning("Timed out waiting for Google Maps to load")
        # Handle consent redirect after direct URL
        if "consent.google" in driver.current_url:
            log.info("EU consent page after direct URL, accepting...")
            self.dismiss_cookies(driver)
            time.sleep(2)
        time.sleep(3)
        self.dismiss_cookies(driver)
