...
/// The pipeline reads unprocessed reviews from reviews_raw, runs RoBERTa + NLI,
/// aggregates into quarterly snapshots, and outputs JSON to stdout.
/// Rust then upserts the snapshots into sentiment_snapshots.
pub async fn fetch(pool: &PgPool, airport: &Airport, _full_refresh: bool) -> Result<FetchResult> {
    ...
    // Check if there are unprocessed reviews for this airport
    let unprocessed: (i64,) = sqlx::query_as(
        "SELECT COUNT(*) FROM reviews_raw r \
         JOIN airports a ON r.airport_id = a.id \
         WHERE a.iata_code = $1 AND (r.processed IS NULL OR r.processed = FALSE)",
    )
    .bind(iata)
    .fetch_one(pool)
    .await?;
    ...
    if unprocessed.0 == 0 {
        info!(airport = iata, "No unprocessed reviews, skipping sentiment pipeline");
    ...
    info!(airport = iata, unprocessed = unprocessed.0, "Running sentiment pipeline");
    ...
    if !output.stderr.is_empty() {
        ...
        for line in stderr.lines() {
            if line.contains("ERROR") || line.contains("Error") {
                warn!(airport = iata, line = %line, "Sentiment pipeline stderr");
            }
        }
    }
    ...
    if !output.status.success() {
        anyhow::bail!(
            "Sentiment pipeline exited with status {} for {}",
            output.status,
            iata
        );
    }
    ...
    info!(airport = iata, records = records, "Sentiment snapshots upserted");
